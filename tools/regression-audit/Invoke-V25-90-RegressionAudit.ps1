...
if (-not (Test-Path -LiteralPath $CandidateRoot -PathType Container)) {
    'V25_90_REGRESSION_AUDIT_FAIL CANDIDATE_ROOT_REQUIRED'
    exit 1
}
...
if ($failures.Count) { $failures | ForEach-Object { "V25_90_REGRESSION_AUDIT_FAIL $_" }; exit 1 }
...
$routes83to90 = @(Get-ChildItem -LiteralPath $contentRoot -Filter '*-learning-route.json' -File | ForEach-Object {
    ...
    if ($null -ne $json.integrationProjection -and [int]$json.integrationProjection.ordinal -ge 83 -and [int]$json.integrationProjection.ordinal -le 90) {
...
$actualFields = @($routes83to90 | ForEach-Object { $_.Json.evidenceLinks | ForEach-Object { $_.PSObject.Properties.Name } } | Sort-Object -Unique)
...
if ($EvidenceRoot) {
    if (-not (Test-Path -LiteralPath $EvidenceRoot -PathType Container)) {
        'V25_90_REGRESSION_AUDIT_FAIL EVIDENCE_ROOT_REQUIRED'
        exit 1
    }
    $EvidenceRoot = (Resolve-Path -LiteralPath $EvidenceRoot).Path
    $gatePath = Join-Path $EvidenceRoot 'run-remediated-portable-90-gate.ps1'
    $portableRoot = Join-Path $EvidenceRoot 'p\single-app-v90-win-x64'
    $zipPath = Join-Path $EvidenceRoot 'Descartes-CertaintyLab-v90-win-x64-portable.zip'
    ...
    if ($failures.Count -eq 0) {
        ...
        Require ([regex]::Matches($gateText,[regex]::Escape('PASS single-app-wiring routes=90 catalogMappings=90 canonicalMappings=22')).Count -eq 1) 'active canonicalMappings=22 signature mismatch'
        Require ([regex]::Matches($gateText,[regex]::Escape('$publishManifest.fileCount -eq 501')).Count -eq 1) 'active publish tree=501 signature mismatch'
        ...
        Require ($treeFiles.Count -eq 501) "portable tree count expected 501; actual $($treeFiles.Count)"
        Require ($manifestLines.Count -eq 501) "manifest count expected 501; actual $($manifestLines.Count)"
        Add-Type -AssemblyName System.IO.Compression
        $zip = [IO.Compression.ZipFile]::OpenRead($zipPath)
        try { $zipFiles = @($zip.Entries | Where-Object Name) } finally { $zip.Dispose() }
        Require ($zipFiles.Count -eq 501) "ZIP entry count expected 501; actual $($zipFiles.Count)"
...
$checkpoint = [ordered]@{
    status=$status
    readOnly=$true
    source=[ordered]@{ jsonFailures=$jsonFailures; routes83to90=$routes83to90.Count; evidenceExtensionMappings=$actualExtensions.Count; derivedCards=$derived.Count; canonicalIds=$canonicalIds.Count; lockFiles=$lockFiles.Count }
...
if (-not $NoReport) {
    ...
    @(
        '# v25/90 回归审计报告','',"状态：``$status``",'',
        "- Source：JSON failures=$jsonFailures；routes #83–90=$($routes83to90.Count)；derived/canonical=$($derived.Count)/$($canonicalIds.Count)。",
