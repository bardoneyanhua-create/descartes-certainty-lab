...
$entry = Join-Path $auditRoot 'Invoke-V25-90-RegressionAudit.ps1'
...
if ($LASTEXITCODE -ne 0 -or ($green -join "`n") -notmatch 'V25_90_REGRESSION_AUDIT_PASS') {
    throw "portable source audit did not pass: $($green -join "`n")"
}

$badEvidence = Join-Path $env:TEMP ('missing-v25-evidence-' + [guid]::NewGuid().ToString('N'))
