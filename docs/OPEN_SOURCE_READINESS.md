...
- 已完成第一轮 3,654 条 evidence record 机器权利盘点；详见 `docs/CONTENT-RIGHTS-AUDIT.md`。
...
1. 按 `docs/CONTENT-RIGHTS-AUDIT.md` 继续完成现代译本、出版社材料和其他第三方内容的逐项权利审计。
2. 审核内容中的待校勘 locator 标记，决定公开前修复或明确标注其实验性状态。
3. 决定公开仓库名称、描述和可见性，并明确授权创建 remote、push 与首个 Release。
4. 在真实用户使用后积累 Issue、反馈、下载和持续维护记录；不得伪造 Star、采用或提交历史。
